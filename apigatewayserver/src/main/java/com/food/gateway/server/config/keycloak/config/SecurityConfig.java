...
public class SecurityConfig {
    ...
    public SecurityWebFilterChain springSecurityFilterChain(ServerHttpSecurity serverHttpSecurity) {
        serverHttpSecurity.authorizeExchange(exchanges -> exchanges.pathMatchers("/**").permitAll()
                        .pathMatchers("/user/api/create/","/user/api/login/").permitAll()
                        ...
                        .pathMatchers("/user/api/update-user/","/user/api/fetch/","/user/api/delete/","/address/user/**").hasRole("user"))
                .oauth2ResourceServer(oAuth2ResourceServerSpec -> oAuth2ResourceServerSpec
                        .jwt(jwtSpec -> jwtSpec.jwtAuthenticationConverter(grantedAuthoritiesExtractor()).jwkSetUri("http://localhost:8072/realms/FoodSwift/protocol/openid-connect/certs")));
